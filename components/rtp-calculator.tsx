...
export function RtpCalculator() {
  ...
  return (
    <section className="rounded-3xl border border-emerald-400/25 bg-emerald-400/10 p-6 sm:p-8">
      ...
      <p className="mt-3 text-neutral-300">Compare the theoretical long-run cost of BGaming&apos;s current 96% public listing and a documented 94% operator example.</p>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">{[96, 94].map((rtp) => <div key={rtp} className="rounded-xl border border-white/10 bg-black/30 p-4"><p className="text-sm text-neutral-400">{rtp}% RTP</p><p className="mt-1 text-xl font-black">{(amount * (1 - rtp / 100)).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p><p className="text-xs text-neutral-500">theoretical house edge</p></div>)}</div>
